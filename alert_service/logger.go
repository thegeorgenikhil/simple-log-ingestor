...
func (l *Logger) LogEntry(level string, message, from string, loggedAt time.Time, email string, mailSent time.Time) {
	...
	entry.Data["mail_sent"] = mailSent.String()
...
func (l *Logger) Info(message, from string, loggedAt time.Time, email string, mailSent time.Time) {
	l.LogEntry("info", message, from, loggedAt, email, mailSent)
}

// Debug logs debug messages with a specific time and from field
func (l *Logger) Debug(message, from string, loggedAt time.Time, email string, mailSent time.Time) {
	l.LogEntry("debug", message, from, loggedAt, email, mailSent)
}

// Error logs error messages with a specific time and from field
func (l *Logger) Error(message, from string, loggedAt time.Time, email string, mailSent time.Time) {
	l.LogEntry("error", message, from, loggedAt, email, mailSent)
}
